    ...
    <div class="gallery-container">
        <?php while ($row = $result->fetch_assoc()) { ?>
            <div class="place-card">
                <?php
                $videoPath = 'videos/' . $row['media'];
                $imagePath = 'images/' . $row['photo'];

                // Check if video file exists and has .mp4 extension
                $videoExt = strtolower(pathinfo($videoPath, PATHINFO_EXTENSION));
                $hasVideo = ($videoExt === 'mp4' && file_exists($videoPath));
                ...
                $hasImage = in_array($imageExt, $imageExts) && file_exists($imagePath);
                ...
                <?php } else { ?>
                    <p><strong>Media not available</strong></p>
                <?php } ?>
